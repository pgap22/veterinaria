...
import { editDiagnosticAxios, deleteDiagnosticAxios, getDiagnosticAxios } from '../../api/vet'

import { useEffect, useState } from 'react'
import { IoIosClose } from 'react-icons/io'

export const ManageDiagnostics = () => {
  const { data, error, isLoading } = useSWR('/diagnosticos', getDiagnosticAxios)
    ...
    <>
      <div className='flex flex-col p-4 h-auto shadow-lg gap-5 rounded-lg'>
        ...
        <div className='flex gap-5'>
          <ModalEditDiagnostic id={data.id} />
...
const ModalDeleteDiagnostic = ({ id }) => {
  ...
  return (
    <>
      <button onClick={() => setShowModal(true)} className='py-3 px-10 bg-red-500 rounded-md text-white hover:bg-red-600 font-medium'>ELIMINAR</button>
      {showModal
        ? (
          <>
            <div
...
const ModalEditDiagnostic = ({ id, data }) => {
  ...
  return (
    <>
      <button onClick={() => setShowModal(true)} className='py-3 px-10 bg-blue-500 rounded-md text-white hover:bg-blue-600 font-medium '>EDITAR</button>
      {showModal
        ? (
          <>
            <div
                  ...
                  <div className='relative p-6 flex-auto'>
                    <form className='bg-white rounded' onClick={handleSubmit(sucessSubmit)}>
                      <div className='mb-4'>
                        ...
                        <textarea rows='4' className='block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300' placeholder='Write your descripcion' {...register('descripcion', { required: true })} />
                      </div>

                      <div className='mb-4'>
                        <label className='block text-gray-700 text-sm font-bold mb-2' htmlFor='name'>Recomendations:</label>
                        <textarea rows='4' className='block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300' placeholder='Write your recomendations' {...register('recomendaciones', { required: true })} />
                      </div>
